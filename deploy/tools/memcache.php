...
$MEMCACHE_SERVERS[] = 'localhost:11211'; // add more as an array - default

$MEMCACHE_LEGEND['localhost:11211']='default';
...
EOB;
		...
		if (!isset($_GET['singleout']) && count($MEMCACHE_SERVERS)>1){
    		foreach($MEMCACHE_SERVERS as $server){
                echo ($i+1).'.';
                if (isset($MEMCACHE_LEGEND[$server])) echo "<b>".$MEMCACHE_LEGEND[$server]."</b>&#160;&#160;";
                echo '<a href="'.$PHP_SELF.'&singleout='.$i++.'">'.$server.'</a><br/>';
    		}
		}
		else{
